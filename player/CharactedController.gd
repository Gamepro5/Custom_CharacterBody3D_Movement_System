...
func uncrouch_check(): #checks if the player can uncrouch. if yes, proceeds to uncrouch the player.
	if crouched:
		...
		uncrouchShape.size = Vector3(originalCollisionHullSize.x-0.001, originalCollisionHullSize.y-0.001, originalCollisionHullSize.z-0.001)
		trace.intersect_groups(Vector3(position.x,position.y+((originalCollisionHullSize.y/2)-crouchHeight/2)+0.001,position.z), uncrouchShape, self, 0x1)
		if trace.hit == false:
			trace.intersect_groups(Vector3(position.x,position.y+0.001,position.z), uncrouchShape, self, 0x1)
			if trace.hit == true:
				position.y += ((originalCollisionHullSize.y/2)-crouchHeight/2)
			#$Torso/Head.position.y = original_eye_height
	...
	if (!noclip):
		...
		if ground_check:
			...
			if (wall_collision_normal != Vector3.ZERO):
						...
						if trace.endpos.y > position.y and trace.normal.angle_to(Vector3.UP) <= max_flr_ang: 
							...
							if ceiling_collision_normal == Vector3.ZERO:
								...
								if on_floor:
									...
									#if wall_collision_normals.size() > 1:
									#	var wall_floor_tangent = wall_collision_normals[0].cross(wall_collision_normals[1]).normalized()
									#	vel = wall_floor_tangent * vel.dot(wall_floor_tangent)
									#	vel = vel.slide(wall_collision_normal)
									#else:
									vel = vel.slide(wall_collision_normal)
				...
				on_wall = false
			if col:
				if (floor_collision_normal != Vector3.ZERO and floor_collision_normal_override != Vector3.ZERO):
					if (ceiling_collision_normal == Vector3.ZERO):
			...
			if col:
				...
				elif (average_normal.angle_to(Vector3.UP) <= max_flr_ang) and !in_water:#floor
					snap_vector = Vector3.DOWN
